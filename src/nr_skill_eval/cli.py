...
from nr_skill_eval.artifacts import create_session_dir
from nr_skill_eval.dataset import DatasetEntry, load_config, load_eval_manifest
...
app = typer.Typer(help="Benchmark a Claude Code skill against an off-the-shelf baseline on a labelled QA manifest.")
...
def _build_judge(cfg: dict) -> Optional[Any]:
    ...
    if not api_key:
        ...
        return None
    try:
        from nr_skill_eval.judge import LLMJudge
    except ImportError as exc:
        typer.echo(f"Judge disabled: failed to import LLMJudge ({exc}). Install nemo-retriever-skill-eval[llm].")
        return None
...
SKILL_CONDITIONS = ("c2_retriever", "c3_retriever_skill")


def _resolve_skill_source(cfg: dict, selected_conditions: list[str]) -> Optional[Path]:
    raw = cfg.get("skill_source_dir")
    needs_skill = any(c in SKILL_CONDITIONS for c in selected_conditions)
    if not raw:
        if needs_skill:
            raise typer.BadParameter(
                "config must set 'skill_source_dir' to the path of the Claude Code skill "
                "directory under test (the folder containing SKILL.md) when running "
                f"conditions {list(SKILL_CONDITIONS)}. Example: "
                "skill_source_dir: ~/code/myproject/.claude/skills/my-skill"
            )
        return None
...
def run_command(
    ...
    skill_source = _resolve_skill_source(cfg, selected)
