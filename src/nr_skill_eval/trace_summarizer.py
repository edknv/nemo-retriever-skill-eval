...
in a neutral temp cwd with ``--setting-sources user`` so project-level skills
or settings don't leak into the summarization session.
"""
...
**Tool-use breakdown** — bulleted list of tool names with counts and one or
two representative invocations each. Keep inputs short.

**Notable patterns** — retries, dead ends, fallback chains, suspicious
behavior. Skip this section if nothing stands out.
...
class TraceSummarizer:
    ...
    def summarize(self, condition: str, domain: str, trace: str) -> str:
        ...
        prompt = _SUMMARIZER_PROMPT_TEMPLATE.format(
            condition=condition, domain=domain, trace=trace
        )
        cmd = [
            "claude",
            "--print",
