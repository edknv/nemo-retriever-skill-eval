...
The transport is abstracted behind the :class:`_ChatClient` Protocol so
this module never imports ``litellm`` directly. ``cli._build_judge``
constructs a ``LiteLLMChatClient`` (defined below) that satisfies the
Protocol; tests pass a stub.
"""
...
if TYPE_CHECKING:
    ...
    from skill_eval.runner import TrialResult
...
class _ChatClient(Protocol):
    """Minimal chat-completion slice this module needs."""
...
class LiteLLMChatClient:
    """Thin litellm wrapper that satisfies the :class:`_ChatClient` Protocol.

    Lives in this module rather than in ``judge.py`` so importing the judging
    surface doesn't drag ``litellm`` in until a client is actually constructed.
    """
    ...
    def __init__(
        ...
        *,
        ...
        try:
            import litellm  # noqa: F401  (presence check; lazy-imported in ``complete``)
        except ImportError as exc:  # pragma: no cover - exercised in install variants
            raise ImportError(
                "LiteLLMChatClient requires `litellm`; install skill-eval[llm]."
            ) from exc
        self.model = model
    ...
    @classmethod
    def from_kwargs(cls, **kwargs: Any) -> "LiteLLMChatClient":
        return cls(**kwargs)

    def complete(self, messages: list[dict], max_tokens: Optional[int] = None) -> tuple[str, float]:
        """Send ``messages`` to the configured judge model. Returns ``(text, latency_s)``."""
        import litellm
...
def evaluate_entry(
    *,
    ...
    if not chosen_path:
        raise ValueError(
            f"judge mode={mode!r} requires "
            f"{('scenario_prompt_path' if mode == 'scenario' else 'simple_prompt_path')}; "
