...
def _aggregate(
    ...
    *,
    ...
    legacy_scores = [
        r.legacy_judge_score for r in query_results if r.legacy_judge_score is not None
    ]
    if legacy_scores:
...
def _md_row(row: dict[str, Any]) -> str:
    ...
    return (
        "| {name} | {sr:.2f} | {r1:.3f} | {r5:.3f} | {r10:.3f} | {judge} | {legacy} "
...
_MAIN_TABLE_HEADER = (
    "| run | success_rate | recall@1 | recall@5 | recall@10 | judge | legacy | q_input | q_output "
